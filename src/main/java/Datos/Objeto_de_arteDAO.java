...
public class Objeto_de_arteDAO {
    ...
    public List<objeto_de_arte> arteArtista(String artista) {
         try {
            objetoDeArtelist = new ArrayList<>();
            ps = con.prepareStatement("SELECT * FROM objeto_de_arte WHERE artista=?");
    ...
    public List<objeto_de_arte> arteExpo(int id) {
        try {
            objetoDeArtelist = new ArrayList<>();
            ps = con.prepareStatement("SELECT * FROM objeto_de_arte "
                    + "JOIN exposicionobjeto "
                    + "ON objeto_de_arte.idobradearte = exposicionobjeto.idobradearte "
                    + "WHERE exposicionobjeto.idexposicion = ?");
            ps.setInt(1, id);
            rs = ps.executeQuery();
            while (rs.next()) {
                objetoDeArte = new objeto_de_arte();
